...
Für die Konzipierung eines RADAR-Systems macht man sich zu Nutze, dass sich ausbreitende elektro-magnetische Wellen an Objekten, auf die sie währen der Ausbreitung treffen, reflektiert werden.
...
Einerseits soll detektiert werden, \emph{ob} ein Objekt von unserem RADAR angeleuchtet wurde und im Idealfall, \emph{wie groß} die zurückgelegte Strecke $d$ ist.
...
\end{equation}
Stellen wir uns vor, dass unser RADAR-System die Sendesequenz $x[\cdot]$ erzeugt, indem es eine endliche Sequenz $\bm x \R^{N}$ wiederholt aussendet, so ist $x[\cdot]$ periodisch mit Periodendauer $N$ und auch $y[\cdot]$ -- nach den ersten $N$ Samples, wenn wir $N$ so groß gewählt haben, dass alle Echos nach der Aussendung von $N$ Samples am Empfänger angekommen sind.
Dies ist gleichbedeudent damit, dass die Sendesequenz länger ist als die Impulsantwort des \gls{lti} Systems, das die Wellenausbreitung in der Umgebung repräsentiert.
...
Wir könnten also einfach ein lokales Maximum in $r_{y,x}[\cdot]$ suchen und dessen Stelle in $d$ umrechnen.

Die naive Berechnung von $r_{y,x}[\cdot]$ für $N$ Werte erfordert in der Größenrdnung $N^2$ viele \glspl{flop}.
Man kann aber ziemlich einfach einsehen, dass man die Kreuzkorrelation als Faltung via
...
Das heißt, dass für die Signalverarbeitung im RADAR effiziente Methoden bereitstehen, solange der Ansatz der Berechnung der Kreuzkorrelation des Empfangssignals mit der Sendesequenz als theoretisch fundiert herausstellt.
%
\begin{listing}[ht]
    \noindent
...
Wie man sieht, hat das Sendesignal einen signifikanten Einfluss auf die \q{Form} der \gls{acf}, welche wiederum einen Einfluss auf die visuelle Qualität der Funktion $r_{x,y}[\cdot]$ hat.
Je stärker ein lokales Maximum ausgeprägt ist, desto besser.
...
Mit einer $\Sinc$-Funktion (\Cref{py:radar1} mitte) belegen wir bekanntermaßen im Frequenzbereich einen ausgedehnten Bereich, was dazu führt, dass wir in der Tat sehr ausgeprägte lokale Maxima bei den wahren $\tau$ erhalten.
...
Gleichzeitig haben diese den Nachteil, dass nicht klar ist, wie man Signale generiert, die sich wie zufällige Sendesignale verhalten, sich aber dennoch kontrolliert und mit einem deterministischen Prozess erzeugen lassen.
...
\end{figure}
%
Wir wollen im Folgenden eine mögliche Art der Sendesequenzerzeugung genauer untersuchen, da sie beide obigen Anforderungen erfüllt.
...
x[n] 
    =   a_1 x[n-1] 
      ...
      + a_L x[n-L],
\]
wobei die Koeffizienten $a_1, \ldots, a_{L} \in \Z_2$, diese als Bits also definieren, welche Werte der Register für die Berechnung des Feedbacks genutzt werden.
...
Demnach ist auch die maximale Periodenlänge mit $2^L-1$ beschränkt.
...
\[
H(z) =  a_1 z^{1} + a_2 z^2 + \ldots a_{L-1} z^{L-1} + a_L z^L,
\]
woran wir wiederum sehen, dass das System nur durch die Wahl der Koeffizienten $a_1, \ldots, a_{L} \in \Z_2$ bestimmt wird.
Man kann zeigen, dass falls die Transferfunktion $H$ ein sogenanntes irreduzibles und primitives Polynom
\footnote{
    ...
    Mehr Infos gibt es in \cite{mceliece1987finite_fields}, wo auch eine explizite Formel für die entstehende Sequenz entwickelt wird.
} ist, so hat die von diesem \gls{lfsr} erzeugte Sequenz maximale Länge, also $2^L-1$.
Das heißt, dass die algebraischen Eigenschaften des $z$-Transformierten Systems, der Transferfunktion $H$, wieder Aufschluss darüber geben, welche Eigenschaften das \gls{lfsr}.
In \Cref{py:mlbs} wird ausgehend von einem Startwert/-zustand des Registers (\texttt{0x1}) und einer sogenannten Tapkonfiguration (\texttt{0xD}), welche den Feedback kontrolliert, eine \gls{mlbs} durch einmaliges Simulieren des \gls{lfsr} erzeugt.
...
\FloatBarrier
%
\subsubsection{\texorpdfstring{\acrshort*{mlbs}}{MLBS} als RADAR-Sendesequenz}
Zum Abschluss wollen wir nun noch untersuchen, wie gut sich die so gewonnenen Sequenzen für RADAR verwenden lassen.
Hierzu betrachten wir die Ausgabe von \Cref{py:radar2}, wo wir das Wissen aus \Cref{py:radar1} und \Cref{py:mlbs} kombiniert haben, um einerseits \q{hardware-effizient} eine Sendesequenz zu erzeugen und andererseits, um die RADAR-Signalverarbeitung \q{rechen-effizient} zu gestalten.
%
\begin{listing}[ht]
    \noindent
    \begin{minipage}{0.51\textwidth}
...
Wie wir sehen, erzeugt das \gls{lfsr} eine Sequenz, welche sehr gut zu unseren Anforderungen passt. 
...
Man kann sogar beweisen, dass für eine Sequenz maximaler Länge $x[\cdot]$ für die \gls{acf} gilt, dass
\begin{equation}
