...
%
%
\subsection{Abtastung von harmonischen Signalen}\label{sec:sample_harm}
%
...
%
%
\subsection{Das Sampling Theorem}
%
...
\[
    x[n] = x_a(nT).
\]
F"ur die bequeme Analyse von kontinuierlichen Signalen und ihrem Anteil an harmonischen Komponenten, benutzen wir die \gls{ft} $\mathcal{F}$, welche ein Signal $x : \R \rightarrow \C$ transformiert in $x \mapsto \mathcal{F} x = X : \R \rightarrow \C$, wobei $X$ definiert durch
%
...
%
definiert ist.
Wir werden versuchen immer die Kombination von Klein- und Gro"sbuchstaben $x \leftrightarrow X$ als \gls{ft}-Paar beizubehalten.
...
\end{equation}
%
Wir wollen darauf hinweisen, dass man diese Art Integral-Transformation rein mathematisch interpretieren kann.
%  und wir betrachten den Raum aller Signale, f"ur welche 
% \[
%     E(x) = \Int{-\infty}{+\infty}{\Abs{x(t)}^2}{t} 
%     = \Int{-\infty}{+\infty}{x(t)x^\ast(t)}{t} < \infty
% \]
% gilt.
% Man nennt diese Signale mit endlicher Energie.
% Wir gehen nun einen Schritt weiter und definieren 
Betrachten wir das Signal $x$ als Vektor in einem Vektorraum, in dem ein \emph{Skalarprodukt} definiert ist.
Starten wir mit etwas bekanntem.
...
\]
%
Beim Bilden von $\ScPr{\bm x}{\bm y}$ k"onnen wir also einen etwas asymmetrischen Standpunkt einnehmen und uns vorstellen, dass wir ermitteln, wie viele \q{Anteile} von $\bm x$ in $\bm y$ zu finden sind.
...
\]
gelten muss.
Setzen wir nun auch f"ur $x[\cdot]$ die inverse \gls{dtft} ein, erhalten wir mit $f = F/F_s$ (siehe \Cref{sec:sample_harm}) und einer Variablentransformation, dass
\[
...
\[
    \frac{1}{F_s}\Int{-F_s/2}{+F_s/2}{
        \exp(-\jmath 2 \pi n F/F_s)
        \exp(\jmath 2 \pi F t)
    }{F} = \frac{\sin(\pi F_s (n - n/Fs))}{\pi F_s (t - n/F_s)}.
\]
...
Au"serdem ist nicht intuitiv klar, weshalb $g_{F_s}$ die behauptete Form haben sollte.
