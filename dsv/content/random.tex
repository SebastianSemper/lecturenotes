...
Wir versuchen nun mit \Cref{thm:random:widrow} aus der Dichte der quantisierten Werte $f_{q(X)}$ die Dichte der nicht-quantisierten Werte $f_X$ zu bestimmen.
Dies ist einmal basierend auf dem vorherigen Beispiel aus \Cref{py:random:quantization2} in \Cref{py:random:quantization3}.
Die Funktion \pyinl{invert_quant} nutzt hierfür die \gls{dft} in Form der \gls{fft} und wendet die Filter im entsprechenden diskreten Spektrum an.
Im vorletzten Schritt wird das Spektrum der charakteristischen Funktion in den Zeitbereich transformiert und schlussendlich der Interpolationskern aus \Cref{thm:sampling_theorem}.
