...
Die Verteilung der nicht-quantisierten Werte ist also auch eine stetige Verteilung, während die Verteilung des quantisierten Signals diskret sein muss -- schließlich ist ja genau das unser Ziel bei der Quantisierung.

Betrachten wir also nun einmal das Area Sampling mit $\Delta Q > 0$ als Abstand zwischen den Quantisierungsstufen analytisch via
\begin{align*}
    f_{q(X)}(x) = \Sum{k \in \Z}{}{
        \delta(x - k \Delta Q) 
...
\end{cases}
\]
Die Faltung von $\Rect_{\Delta Q}$ mit einer Funktion $f$ ergibt
\begin{equation}
...
Diese Auswertung kann wegen \Cref{eq:random:density} durch Verwenden Verteilungsfunktion $F_X$ erfolgen.
...
Dann vollziehen wir die beiden Möglichkeiten für die Berechnung der Dichte der quantisierten Werte $f_{q(X)}$, indem wir erst quantisieren und dann das Histogram an den Quantisierungsstufen bilden, um \pyinl{XN_f_q} zu erhalten.
...
Wie man im unteren Plot erkennen kann, stimmen die beiden Implementierungen sehr gut überein. 
...
Unter welchen Bedingungen kann man aus der diskreten Dichte $f_{q(X)}$ die ursprüngliche Dichte $f_{X}$ des Nicht-quantisierten Signals rekonstruieren?

Hierzu betrachten wir den Zusammenhang aus \Cref{eq:random:area_sampling} im Frequenzbereich.
In \eqref{eq:random:char_fun} haben wir die charakteristische Funktion $\phi_X$ als Fourier-Transformierte einer Dichte $f_X$ eingeführt.
Definieren wir $F_Q = 1 / \Delta Q$, dass ergibt sich aus den Eigenschaften der Fourier-Transformation direkt, dass
\begin{equation}
...
Da die Dichte der quantisierten Werte aus Faltung und Abtastung mit Rate $F_Q = 1/T_Q$ hervorgeht, ist deren Spektrum periodisch mit Periodenlänge $T_Q$. 
...
Wir kommen nun zum Abschluss, da wir nun basierend auf \eqref{eq:random:area_sampling_fourier} ein Kriterium angeben können, sodass wir $f_X$ aus $f_{q(X)}$ rekonstruieren können. 
...
Man kann sich in diesem Sinne Quantisierung als \q{Abtastung} der Dichte $f_X$ vorstellen.
...
\begin{listing}[ht]
    ...
    \begin{minipage}{0.51\textwidth}
        \strut\vspace*{-\baselineskip}\newline
        \inputminted[firstline=80, lastline=115]{python3}{code/random/quantization3.py}
    \end{minipage}%
    \begin{minipage}{0.48\textwidth}
        \strut\vspace*{-\baselineskip}\newline
