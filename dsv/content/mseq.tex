...
In \Cref{py:radar1} sind einige verschiedene Sendesignale miteinander verglichen, indem zwei Reflexionen mit verschiedenen Abst"anden simuliert werden und aus dem resultierenden Empfangssignal $y[\cdot]$ wiederum $r_{x,y}$ bestimmt wird.
Wie man sieht, hat das Sendesignal einen signifikanten Einfluss auf die \q{Form} der \gls{acf}, welche wiederum einen Einfluss auf die visuelle Qualit"at der Funktion $r_{x,y}[\cdot]$ hat. 
...
Mit einer $\Sinc$-Funktion (\Cref{py:radar1} mitte) belegen wir bekannterma"sen im Frequenzbereich einen kontinuierlichen Bereich, was dazu f"uhrt, dass wir in der Tat sehr ausgepr"agte lokale Maxima bei den wahren $\tau$ erhalten.
