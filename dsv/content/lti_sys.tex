...
\[
y[n] = \frac{1}{n+1}\Sum{k=0}{n}{x[k]},
\]
was ein kumulatives Mittel von $0$ bis $n$ darstellt.
Wichtig ist hier, dass dies ein Beispiel für ein sogenanntes \gls{ltv}-System.
Wegen der Linearität existiert immernoch eine Impulsantwort, die durch Faltung mit dem Eingang $x[\cdot]$ den Ausgang $y[\cdot]$ ergibt, diese ist aber zeitvariant, also $h_n[\cdot]$.
...
Man sieht, dass diese Umformulierung deutlich macht, dass wir nur $y[n-1]$ im Speicher halten müssen und dieses mit einer Art \emph{zeitverzögertem Feedback} ausstatten müssen.
Hierbei ist natürlich die Zeitverzögerung essenziell, denn müssten wir $y[n]$ in Abhängigkeit von $y[n]$ berechnen, würde uns das vor ein halbwegs unlösbares Problem stellen.
...
Je nachdem wie schnell $h[\cdot]$ gegen $0$ konvergiert, benötigen wir mehr oder weniger von $0$ verschiedene Einträge in $h_{\rm FIR}$, was sich auf den Implementierungsaufwand auswirkt.
...
\begin{listing}
    ...
    \end{minipage}
    \codecaption{dsv/code/exp_mean.py}{Approximation eines exponentiellen Mittelwertes, also eines \acrshort*{iir}-Systems, durch ein \acrshort*{fir}-System}\label{py:exp_mean}
\end{listing}
