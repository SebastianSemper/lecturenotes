...
\end{equation}
auch wieder ein Parseval Theorem für die \acrlong{ft} ergibt, was aber in diesem Fall als Plancherel Theorem\linkfootnote{{https://en.wikipedia.org/wiki/Plancherel_theorem}} bekannt ist.
...
Die vorher eingeführten Transformationen sind zwar hilfreich für theoretische Argumentation, wie beispielsweise beim Sampling-\Cref{thm:sampling_theorem}, aber nicht für praktische Anwendungen. 
...
\]
an.
Damit sind bereits beide Eigenschaften (Diskretheit und Periodizität) des Signals \q{eingepreist}.
...
\]
Das heißt, dass wir das periodische Signal $x[\cdot]$ mit dem \emph{endlich-dimensionalen} Vektor $\bm x \in \C^{N}$ identifizieren, wir setzen also die Einträge des Vektors als $\bm x_i = x[i-1]$.
Dann können wir auch für das entsprechende Skalarprodukt der Vektoren $\bm x_{1,2} \in \C^N$ als
\[
\ScPr{\bm x_1}{\bm x_2} 
...
Wie in \Cref{sec:fourier:cont:period} finden wir, dass also $\ScPr{\bm x_k}{\bm x_\ell} = 0$ gilt, falls $k \neq \ell$ und $\ScPr{\bm x_k}{\bm x_k} = N$.
...
\]
gilt.
Das heißt, dass sich Fourier-Analyse und Fourier-Synthese von diskreten und periodischen Signalen durch eine Matrix-Vektor-Multiplikation durchführen lässt!
...
Vor allem moderne \glspl{cpu} und \glspl{gpu} sind auf diese Art von Operation sehr stark optimiert indem spezielle Teile der Architektur nur für diese Operation bereitgestellt werden.
...
\end{equation}
Dann können wir analog zu vorher herleiten, dass für
\[
...
%
%
\subsection{Fourier-Transformation diskreter aperiodischer Signale}\label{sec:fourier:disc_aperiod}
%
...
Hierzu definieren wir die zugehörige \gls{ft}, die wir \gls{dtft} nennen, durch
\begin{equation}\label{eq:fourier:dtft}
    X(f) = \Sum{n \in \Z}{}{x[n] \exp(-\jmath 2 \pi f n)},
\end{equation}
genau wie in \Cref{sec:sampling} bei der Herleitung von \eqref{thm:sampling_theorem}.
Im Unterschied zur \q{analogen} \gls{ft} \eqref{eq:fourier:fourier_trafo} sehen wir, dass $X$ nur für Frequenzen $f \in (0,1]$ definiert ist, da das diskrete Signal $x_f[n] = \exp(\jmath 2 \pi f n)$ periodisch in $f$ ist.
...
Bis auf das Vorzeichen in der Funktion $\exp(\cdot)$ gleicht \eqref{eq:fourier:dtft} einer Fourier-Reihe der periodischen Funktion $X$.
...
\end{equation}
wiederfinden.
Wir sind hier also den umgekehrten Weg gegangen und haben $X$ als eine periodische Funktion im Frequenzbereich in ihre Fourier-Reihe entwickelt, welche dann wiederum Informationen über den entsprechend diskreten Zeitbereich liefert.
...
Dies liegt daran, dass die \gls{dtft} von $x[\cdot]$ nicht konvergiert, wie wir in \Cref{py:fourier_series} bereits gesehen hatten -- zumindest nich in dem üblichen Sinn.
...
Doch dies allein ist nicht ausreichend, denn wir benötigen noch einen effizienten Weg, um zwischen Zeit- und Frequenzbereich wechseln zu können.
...
Die \gls{fft} erlaubt es aber eine \gls{dft}  in der Größenordnung von nur $N \log{N}$ \glspl{flop} durchzuführen.
...
Wie man sieht, ist das Ergbenis der Faltung im Zeitbreich mit dem im Frequenzbereich identisch.
...
Jedoch sieht man auch, dass sich die Phase des Resultates von der des Eingangs unterscheidet.
...
Darüber hinaus ist unsere Implementierung eine \q{offline}-Version in dem Sinne, dass die zu verarbeitenden Daten bereits vollständig im Speicher liegen müssen.
Doch wenn man an Echtzeitanwendungen, beispielsweise im Audiobereich, denkt so wird es notwendig sein, diese Operation auf Daten anzuwenden, die nicht vollständig im Zugriff liegen, sondern sich fortlaufend aktualisieren.
Außerdem will man natürlich nicht immer eine zyklische Faltung, sondern eine \q{normale} Faltung implementieren, besonders im Echtzeitfall ist dies besonders wichtig.

All diese Probleme können aber mit einigen Tricks so umformuliert werden, dass man an einer Stelle in der Verarbeitung wieder eine zyklische Faltung rechnen kann.
...
Wir haben nun genügend Eigenschaften der \gls{dft} gesammelt, sodass wir uns nun Anwendungen zuwenden können.
